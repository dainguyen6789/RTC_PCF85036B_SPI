...
//	float rx_pos_12h=a*pow(dd,3)+b*pow(dd,2)+c*dd+d;// pow (base, power)
	...
	AT25SF041_WriteEnable();
	//Wait_ms_SPINOR(100);
	AT25SF041_Write(Byte_Page_Program, 4,0x32);		
	Wait_ms_SPINOR(100);
	
	AT25SF041_WriteEnable();
	//Wait_ms_SPINOR(100);
	AT25SF041_Write(Byte_Page_Program, 5,0x32);
	Wait_ms_SPINOR(100);
	
	AT25SF041_WriteEnable();
	//Wait_ms_SPINOR(100);
	AT25SF041_Write(Byte_Page_Program, 6,0x32);
	Wait_ms_SPINOR(100);
	
	AT25SF041_WriteEnable();
	//Wait_ms_SPINOR(100);
	AT25SF041_Write(Byte_Page_Program, 7,0x32);	
	Wait_ms_SPINOR(100);

	AT25SF041_WriteEnable();
	//Wait_ms_SPINOR(50);
	AT25SF041_Write(Byte_Page_Program, 8,0x32);	
	Wait_ms_SPINOR(100);
					...
					if(iUse_prevday_calib_value==0)// FIRST day of calibration
					{
							...
							if(BCDtoDec1(mins)%calib_stamp==0 &&  BCDtoDec1(seconds&0x7f)==0 && sunlight_ADC>=sunlight_ADC_Threshold)
							{
								if(BCDtoDec1(hours)<=16  && BCDtoDec1(hours)>=7)
								{
									...
									dat_to_store.calib_max_voltage_ADC=1;
									...
									dat_to_store.light_ADC=2;
									
									dat_to_store.Voltage_at_LUT_pos=3;
