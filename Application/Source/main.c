...
//	float rx_pos_12h=a*pow(dd,3)+b*pow(dd,2)+c*dd+d;// pow (base, power)
	...
	while(1)
	{
		...
		//if (count==20)
		{
			...
			{
				Step_move(107, direction);// 1.8* step angle, 200 steps ~ 1 round, 119 steps ~ 1mm movement, l(mm)=step*pi/337.5, L=R1*R3/R2*pi*n/(100*27), R1 is the pulley attached to the motor, R2 is the pulley attached to the long shaft with timing belt, R3 is the long pulley 
				if (direction==1)
				{
